...
bool sortByHeight( const vector<int>& v1,
              const vector<int>& v2 ) {
    return v1[2] < v2[2];
}

void ByShortest::setPaintings(vector<vector<int>> paintingVector) {
    vector<vector<int>> temp;
    temp = paintingVector;

    sort(temp.begin(), temp.end(), sortByHeight);

    paintings = temp;
}
vector<vector<int>> ByShortest::getPaintings() {
    return paintings;
}
void ByShortest::setTotalPrice(int total) {
    totalPrice = total;
}
int ByShortest::getTotalPrice() {
    return totalPrice;
}


vector<vector<int>> ByShortest::Shortest(vector<vector<int> > paintings, int rows, int cols) {
    ...
    ofstream output("../output/testing-custom.txt");
